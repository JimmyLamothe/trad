...
with open(filename_txt, 'w') as txt_output:
    title_list = []
    input_length = len(title_list)
    input('Number of titles: ' + str(input_length))
...
with open(filename_txt, 'r') as txt_input:
    ...
    output_length = len(output_clip_list)
    print('Number of titles: ' + str(output_length))
    input('Match with input: ' + str(input_length == output_length)+ '. Continue?')
