"""Takes a .srt list of subtitles and (optionally) a .xml corresponding to a FCP-format
...
2. name of .xml file to be output (do not include .xml at end).
3. (optional) location of .xml file - use if given, don't use if program crashes
...
try:
    input_seq = sys.argv[3] #.xml FCP-format file containing input sequence.
except IndexError:
    input_seq = 'input/XML_TITRE.xml'
...
with open(input_srt, 'r') as srt_source:
    ...
    for i in range(len(lines)):
        ...
        if lines[i][13:16] == '-->':
            ...
            frame_in = math.floor(int(lines[i][9:12])/1000*30)
            hour_out = int(lines[i][17:19])
            min_out = int(lines[i][20:22])
            sec_out = int(lines[i][23:25])
            frame_out = math.floor(int(lines[i][26:29])/1000*30)
            TC_in = (hour_in, min_in, sec_in, frame_in)
            TC_out = (hour_out, min_out, sec_out, frame_out)
            duration_in = tc_calc.tc_calc(TC_in, reverse = True)
            duration_out = tc_calc.tc_calc(TC_out, reverse = True)
            # Parse Title Text
